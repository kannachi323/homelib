...
type ChannelTaskMap = Set<string>;
type ChannelMap = Record<string, ChannelTaskMap>;

type ChannelState = {
  channels: ChannelMap;

  addTask: (channelName: string, taskId: string, callback: () => void) => void;
  removeTask: (channelName: string, taskId: string, callback: () => void) => void;

  joinTransferChannel: () => void;
  createTransferTask: (task: string, srcClientID: string, dstClientID: string, totalChunks: number) => string | null;
...
export const useChannelStore = create<ChannelState>((set, get) => ({
  ...
  addTask: (channelName, taskId, callback) => {
    set((state) => {
      const existingSet = state.channels[channelName] ?? new Set<string>();
      ...
      return {
        channels: {
          ...state.channels,
          [channelName]: updatedSet,
        },
      };
    });
  },

  removeTask: (channelName, taskId, callback) => {
    set((state) => {
      const currentSet = state.channels[channelName];
      if (!currentSet) return { channels: state.channels };
  ...
  createTransferTask: (task, srcClientID, dstClientID, totalChunks) => {
    const client = useClientStore.getState().client;
    if (!client) throw new Error("Client is not defined");
    const conn = useClientStore.getState().conn;
    if (!conn) throw new Error("WebSocket connection is not defined");
  ...
  handleChannelResponse: (data: string) => {
    ...
    switch (res.task) {
      ...
      case TransferTask.UploadStart:
        ...
        addTask(res.channel_name, res.task_id, () => {});
        break;

      case TransferTask.UploadComplete:
        console.log(`Upload completed for taskID: ${res.task_id}`);
        removeTask(res.channel_name, res.task_id, () => fetchFiles(currentPath));
        break;
    }
  },
